...
class DatasetGenerator():
    ...
    def load_data(self, DIR):
        ...
        """
        for filename in glob(os.path.join(DIR, '*.wav')):
            wav_files = filename
            show = wav_files.split('\\')
            name = show[3]
            ...
            show = name.split('.')
            label = re.sub(r"[@#%$^()1234567890-Copy]", "", show[0])
            if label in self.label_set:
                label_id = self.text_to_labels(label)
                fle = os.path.join(DIR, name)
                sample = (label, label_id, name, fle)
                data.append(sample)
        f.close()
        """
...
DIR = ["D:\Speech_GP\داتابيز","D:\Speech_GP\سؤال عن الحال","D:\Speech_GP\قراءه التقرير الطبي","D:\Speech_GP\سؤال الدكتور","D:\Speech_GP\رفض","D:\Speech_GP\رعايه نفسيه","D:\Speech_GP\نعم","D:\Speech_GP\اعتذار","D:\Speech_GP\اوامر","D:\Speech_GP\التعريف بالنفس","D:\Speech_GP\وقت البدايه"]     # train and test data
...
def predict():
    ...
    y_pred_proba = model.predict_generator(dsGen.generator(BATCH, mode='prediction'),
                                           int(np.ceil(len(dsGen.df_test) / BATCH)), verbose=1)
